...
export class CartDetailService {
  ...
  getRecordDetails(recordId: number): Observable<IRecord | null> {
    return this.http.get<IRecord>(`${this.urlAPI}records/${recordId}`).pipe(
      map((record: IRecord) => ({
        ...record,
        ImageUrl: (record as any).imageRecord || record.ImageUrl, // Ensure ImageUrl is populated
      })),
      catchError((error) => {
  ...
  getCartDetailsByEmail(email: string): Observable<ICartDetail[]> {
    return this.http
      .get<any>(`${this.urlAPI}cartdetails/getCartDetails/${email}`)
      .pipe(
        map((response) => {
          let cartDetailsArray: ICartDetail[];

          if (Array.isArray(response)) {
            cartDetailsArray = response;
          } else if (response && response.$values) {
            cartDetailsArray = response.$values;
          } else {
            console.warn(
              'Unexpected API response structure for cart details:',
              response
            );
            cartDetailsArray = [];
          }

          return cartDetailsArray.map((detail) => ({
            ...detail,
            record: detail.record
              ? {
                  ...detail.record,
                  ImageUrl:
                    (detail.record as any).imageRecord ||
                    detail.record.ImageUrl,
                }
              : detail.record,
          }));
        }),
        catchError((error) => {
          console.error('Error getting cart details by email:', error);
          return throwError(() => error);
        })
      );
  }
