...
export class RecordsService {
    ...
    return this.http.get<any>(`${this.urlAPI}records`, { headers }).pipe(
      map((response) => {
        ...
        return Array.isArray(records)
          ? records.map((record) => ({
              ...record,
              ImageUrl: record.imageRecord || record.ImageUrl, // Use existing ImageUrl or map from imageRecord
            }))
          : [];
      }),
      tap((records) => {
  ...
  addRecord(record: IRecord): Observable<IRecord> {
    ...
    return this.http
      .post<any>(`${this.urlAPI}records`, body, {
        headers,
      })
      .pipe(
        map((response) => {
  ...
  updateRecord(record: IRecord): Observable<IRecord> {
    ...
    return this.http
      .put<any>(`${this.urlAPI}records/${record.idRecord}`, body, { headers })
      .pipe(
        map((response) => {
        ...
        map((response) => {
          ...
          if (Array.isArray(response)) {
            records = response.map((record) => ({
              ...record,
              ImageUrl: record.imageRecord || record.ImageUrl,
            }));
          }
          // Handle $values wrapper
          else if (response && response.$values) {
            records = response.$values.map((record: any) => ({
              ...record,
              ImageUrl: record.imageRecord || record.ImageUrl,
            }));
          }
            ...
            records = [
              {
                ...response,
                ImageUrl:
                  (response as any).imageRecord || (response as any).ImageUrl,
              },
            ];
          }
