...
export class ListrecordsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Subscribe to stock updates - this part should remain for all users
    ...
    // Subscribe to cart updates - this part should remain for all users
    ...
    this.resizeObserver = new ResizeObserver((entries) => {
      for (const entry of entries) {
        if (entry.target.classList.contains('record-item')) {
          // Perform actions when a .record-item changes size or visibility
          this.updateListVisuals();
        }
      }
    });
